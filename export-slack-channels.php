...
function format_channels_list($data) {
	...
	$fmt = ('|%-90s|%-150s|%-52s|%s|' . PHP_EOL);
	$header = sprintf($fmt, 'Channel Name', 'Channel Purpose', 'Channel Topic', 'First Message');
	...
	foreach($data->channels as $chan) {
		...
		$first_msg = strftime("%Y-%m-%d %H:%M:%S %z", $history->messages[0]->ts);

		printf($fmt,
			'[[https://team.slack.com/messages/' . $chan->name . '|#' . $chan->name . ']]',
			markup($chan->purpose->value), $topic, $first_msg
		);
	}
